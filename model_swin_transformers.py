...
from swinTransformerV1 import * ## Encoder
from upernet import *			## Decoder
...
class Net(nn.Module):
	...
	def __init__(self, 
		submission=False, 
		arch='swin_small_patch4_window7_224_22k', 
		extendLevel=False,
		decoder_cfg=dict(),
		):
		...
		self.rgb = RGB()
		self.arch = arch #'swin_small_patch4_window7_224_22k' / 'swin_tiny_patch4_window7_224'
		...
		#[96, 192, 384, 768]
	 	decoder_dim = decoder_cfg.get('decoder_dim', 256)
		self.decoder = UPerDecoder(
			in_dim=encoder_dim,
			ppm_pool_scale=[1, 2, 3, 6],
			ppm_dim=512,
			fpn_out_dim=decoder_dim
		)
